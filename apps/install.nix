...
let
  ...
  in pkgs.writeShellApplication {
    ...
    text = ''
      ...
      zpool create -f -O encryption=on -O keyformat=passphrase -O keylocation=prompt -O compression=on -O mountpoint=none -O xattr=sa -O acltype=posixacl "${zfsPool}" "$DISK_PART_ROOT"
      #zpool create -f \
       # -o ashift=12 \
        #-o autotrim=on \
        #-O canmount=off \
